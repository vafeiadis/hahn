Require Import NPeano Omega Setoid HahnBase.
...
Proof.
  ...
  replace (S n * (S (S n))) with (2 * (S n) + n * (S n)).
    rewrite div2_add_double; omega.
  rewrite (Nat.mul_comm _ (S (S n))); simpl; omega.
Qed.  
    
Lemma arith_sum_double n : arith_sum n * 2 = n * S n.
Proof.
  induction n; try done.
  rewrite arith_sum_S, (Nat.mul_comm _ (S (S n))); simpl; omega.
Qed.
...
Proof.
  ...
  rewrite arith_sum_S.
  forward apply (arith_sum_le_mono n (n + k)); omega.
Qed.

Lemma arith_sum_lt_mono n m (L : n < m) : arith_sum n < arith_sum m.
Proof.
  forward apply (arith_sum_lt n m); omega.
Qed.

Lemma arith_sum_inj n m (EQ: arith_sum n = arith_sum m) :
  n = m.
Proof.
  destruct (lt_eq_lt_dec n m) as [[A|A]|A]; desf; try omega.
  all: apply arith_sum_lt in A; omega.
Qed.
...
Lemma nat_tup_inj n1 n2 m1 m2
  ...
  n1 = m1 /\ n2 = m2.
Proof.
  eapply arith_sum_inj_gen in EQ; omega.
Qed.
